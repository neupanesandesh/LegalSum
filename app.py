...
def main():
    ...
    if st.session_state["authentication_status"]:
        ...
        if app_mode == "Legal Decision Summarizer":
            ...
            # Initialize session state for processed text if not exists
            ...
            if choice1 == 'Copy-Paste Text':
                ...
                if user_file_input is not None:
                    # Only process if file has changed or text hasn't been processed
            ...
            if show_additional_inputs and st.session_state.processed_text is not None:
                ...
                if st.button("Summarize"):
                    if state == "New Jersey":
                        ...
                        summary = text_summarizer_alternate(st.session_state.processed_text)
                        
                        # Process all the necessary information for New Jersey
                        summary = summary.replace("District Court", "district court")
                        
                        # Get federal/state classification
                        federal_response = client.chat.completions.create(
                            model=GPTModel,
                            temperature=0.2,
                            max_tokens=16,
                            messages=[
                                {"role": "system", "content": """Determine if the legal case, if related to a state or federal case, the federal cases are these 
                        ...
                        court_response = client.chat.completions.create(
                            ...
                            messages=[
                                {"role": "system", "content": prompt_court_option},
                                {"role": "user", "content": st.session_state.first_two_pages}
                            ]
                        )
                        ...
                        # Add title
                        title_case = f"*{title(st.session_state.first_two_pages)}*"
                        summary = title_case + ", " + summary
                        
                        # Process taxonomy
                        prompt_taxonomy = """[Your existing taxonomy prompt]"""  # Include the full taxonomy prompt here
                        ...
                        st.subheader("Summary:")
                        st.write(Connecticut_summarizer(st.session_state.processed_text))
                    elif state == "Texas":
                        st.subheader("Summary:")
                        st.write(Texas_summarizer(st.session_state.processed_text))
                    else:
